...
function commitRootOrgInfo(){
	...
	if(contactName==undefined || contactName==""){
		commonUtil.showPopup("提示","联系人姓名不能为空");
		return;
	}
	...
	if(mobilePhone==undefined || mobilePhone==""){
		commonUtil.showPopup("提示","移动电话不能为空");
		return;
	}
	...
	if(telephone==undefined || telephone==""){
		commonUtil.showPopup("提示","固定电话不能为空");
		return;
	}
	...
	if(idCard==undefined || idCard==""){
		commonUtil.showPopup("提示","法人身份证号不能为空");
		return;
	}
	...
	if(!$("#isCheck").is(":checked")){
		commonUtil.showPopup("提示","请同意电子协议");
		return;
	}
	
	//20、提交主机构更新请求
	$.ajax({
		url : basePath + "/orgInfo/updateRootOrgInfo",
		asyn : false,
		type : "POST",
		data : $('#root_org_info_form').serialize(),
		dataType : "json" ,
 ...
 */
function initCancelRootOrgPage(){
	//主机构名称初始化
	var orgName = $.trim($("#orgName").val());
	$("#cancel_org_name").html(orgName);
	
	//注销原因初始化
	$("#cancel_reason").val("");
	
	//注销附件初始化
	$("#cancel_upload_org_img").attr("src",basePath+"/static/images/common/addimg.png");
	$("#cancel_org_img").val("");
}
 ...
 * @author chengzhihuan 2017年5月23日
 */
function commitCancelRootOrg(){
	//注销原因
	var cancelReason = $.trim($("#cancel_reason").val());
	if(cancelReason==undefined || cancelReason==""){
		commonUtil.showPopup("提示","主机构注销原因不能为空");
		return;
	}
	
	if(cancelReason.length>160){
		commonUtil.showPopup("提示","主机构注销原因不能超过80个汉字");
		return;
	}
	
	//注销附件
	var attachment = $.trim($("#cancel_org_img").val());
	if(attachment==undefined || attachment==""){
		commonUtil.showPopup("提示","主机构注销附件不能为空");
		return;
	}
	
	//提交注销申请
	$.ajax({
		url : basePath + "/orgInfo/applyCancelRootOrgInfo",
		asyn : false,
		type : "POST",
		data : {"cancelReason":cancelReason, "attachment":attachment},
		dataType : "json" ,
		success : function(dataStr) {
			if(dataStr){
				if(dataStr.success){
					commonUtil.showPopup("提示","提交注销申请成功");
				...
				commonUtil.showPopup("提示","提交注销申请服务异常忙，请稍后重试");
				return;
			}
		}
	});
