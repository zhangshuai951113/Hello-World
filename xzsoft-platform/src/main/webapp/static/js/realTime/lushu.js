...
(function() {
    ...
    var LuShu =
        ...
        BMapLib.LuShu = function(map, path, opts) {
            ...
            //this._getPath = opts.getPath; //zhangxb 添加
        }
    /**
    ...
    baidu.object.extend(LuShu.prototype, {
        ...
        _move: function(initPos, targetPos, effect) {
            ...
            me._map.addOverlay(new BMap.Polyline(pointsArr, {
                strokeColor: "#7fde8a",
                strokeWeight: 5,
                strokeOpacity: 1
            })); // 画线  
         ...
         * @time 20170824
         * @desc 是否需要调用getPath方法
         */
        _needGetPath: function() {
            var me = this;
            if (me._path.length > 0) {
                me.i = 0;
                me._moveNext(0);
            }
            /*me._getPath(); //zhangxb 添加
            if (me.success==1) {//正常运行
                if (me._path.length > 0) {
                    me.i = 0;
                    me._moveNext(0);
                }
            } else if(me.success==2) {//暂停
                setInterval(function() {
                    me._needGetPath();
                }, 10000);
            }else if(me.success==3) {//到站，暂不执行任何操作
