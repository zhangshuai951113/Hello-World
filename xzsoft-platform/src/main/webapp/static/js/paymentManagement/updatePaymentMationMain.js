$(function(){
	var proxyPerson=$.trim($("#proxyPerson").val());
	if(proxyPerson!=null && proxyPerson!=''){
		$("#proxy_person").show();
	}
	
	//判断选择的是结算还是对账
	var settOrAcc=$.trim($("#settOrAcc").val());
	if(settOrAcc==2){
		$("#settDiv").hide();  //结算框隐藏
		...
		var accIds=$("#accountCheckInfoIds").val();
		var sArr = accIds.split(",").length;
	    if(sArr>1){
		...
		var settIds=$("#settlementInfoInfoId").val();
		var sArr = settIds.split(",").length;
	    if(sArr>1){
	    	$("#thisPayPrice").attr("readonly",true);
	    }else if(sArr==1){
	    	$("#thisPayPrice").attr("readonly",false);
...
//点击取消按钮
function closeUpdatePaymentInfo(){
	window.location.href=basePath+"/paymentInfo/goPaymentInfoMationPage"+"?sign=1";
}

//是否代理付款切换
/*function switchIsProxyPayment(){
	$(':input','#update_payment_info_form') 
...
//点击保存按钮
function updatePaymentInfoMation(){
	//验证数字
	var regax=/^[0-9]*$/;
	...
	var settOrAcc=$.trim($("#settOrAcc").val());
	if(settOrAcc==2){
		var accountCheckInfoId=$.trim($("#accountCheckInfoIds").val());
	...
	$.ajax({
		...
		data:$("#update_payment_info_form").serialize()+"&sign="+2+"",
		dataType:"json",
		type:"post",
		async:false,
