...
function list(number){
	...
	var driverStatus=$.trim($("#driverStatus").val());
	$.ajax({
		url:"findAllTDriver",
		...
		success:function(resp){
			...
			$.each(objs,function(index,ele){
				...
				if(ele.isAvailable==1){
					...
					tr+="			<div class='delete-operation operation-m'  onclick='deleteDriverAvailable(\""+ele.id+"\")'><div class='disable-icon'></div><div class='text'>删除</div></div>";
					tr+="			<div class='disable-operation operation-m'  onclick='updateDriverAvailable(\""+ele.id+"\")'><div class='disable-icon'></div><div class='text'>停用</div></div>";
					tr+="		</td>";
					tr+="</tr>";
				}else{
					tr+="		<td class='operation'>";
					tr+="           <div class='modify-operation operation-m'  onclick='updateDriver(\""+ele.id+"\")'><div class='modify-icon'></div><div class='text'>修改</div></div>";
					tr+="			<div class='delete-operation operation-m'  onclick='deleteDriverAvailable(\""+ele.id+"\")'><div class='disable-icon'></div><div class='text'>删除</div></div>";
					tr+="			<div class='enabled-operation operation-m'  onclick='updateDriverAvailable(\""+ele.id+"\")'><div class='enabled-icon'></div><div class='text' style='color:#8fc31f'>启用</div></div>";
					tr+="		</td>";
					tr+="</tr>";
				}
				//将tr追加
...
function updateDriver(id){
	...
	$.ajax({
		 ...
		 success:function(resp){
			 if(resp.success){
				var url=basePath+"/driverInfo/updateTDriverPage";
				$("#show_department_info").load(url,function(){
					//上传图片初始化
					$('.upload_img').each(function(){
						uploadLoadFile($(this));
