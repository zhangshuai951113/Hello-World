$(function(){
    ...
    biddingCoverFrozenList(1);
  //全选/全不选
	$("body").on("click",".all_bidding_cover_frozen",function(){
		if($(".all_bidding_cover_frozen").is(":checked")){
			//全选时
			$(".sub_bidding_cover_frozen").each(function(){
...
//投标被冻结信息全查
function biddingCoverFrozenList(number){
	...
	$.ajax({
		...
		success:function(resp){
			...
			$.each(obj,function(index,ele){
				...
				tr+="<td><label class='i-checks'><input name='cId' id='id' type='checkbox' class='sub_bidding_cover_frozen' data-id=\""+ele.id+"\" ></label></td>";
				tr+="<td>"+(index+1)+"</td>";
				...
				$("#biddingCoverFrozenList").append(tr);
			});
		}
	});
	$.ajax({
		...
		success:function(resp){
			...
			$("#bidding-cover_frozen").text("搜索结果共"+resp+"条");
		}
	});
}
...
var pagination = $(".pagination-list").operationList({
	  ...
	  "chagePage":function(current){
	    //调用ajax请求拿最新数据
		  biddingCoverFrozenList (current);
	  }
	});
...
//重置招标被冻结信息
function resetBiddingCoverFrozenList(){
	setTimeout(function(){
		biddingCoverFrozenList(1);
	},500);
}
...
//招标冻结信息导出
function biddingCoverFreezeInformationExport(){
    var ids=getCapIds();
	...
	var url = basePath+ "/fundAccountManagement/biddingFreezeInformationExport?ids="+ids+"&sign="+1;
