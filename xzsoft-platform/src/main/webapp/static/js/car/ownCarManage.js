...
function searchCarInfo(number) {
	...
	// 营运证到期日期
	var operationEndTimeStart = $.trim($("#operationEndTimeStart").val());
	var operationEndTimeEnd = $.trim($("#operationEndTimeEnd").val());
	// 商业险到期日期
	var businessInsuranceEndTimeStart = $("#businessInsuranceEndTimeStart").val();
	var businessInsuranceEndTimeEnd = $("#businessInsuranceEndTimeEnd").val();
	// 交强险到期日期
	var trafficInsuranceEndTimeStart = $("#trafficInsuranceEndTimeStart").val();
	var trafficInsuranceEndTimeEnd = $("#trafficInsuranceEndTimeEnd").val();

	// 请求地址
	var url = basePath + "/carInfo/listOwnCarInfoData #own_car_info";
	$("#search_car_info").load(url, {
		...
		"operationEndTimeStart" : operationEndTimeStart,
		"operationEndTimeEnd" : operationEndTimeEnd,
		"businessInsuranceEndTimeStart" : businessInsuranceEndTimeStart,
		"businessInsuranceEndTimeEnd" : businessInsuranceEndTimeEnd,
		"trafficInsuranceEndTimeStart" : trafficInsuranceEndTimeStart,
		"trafficInsuranceEndTimeEnd" : trafficInsuranceEndTimeEnd
	}, function() {
		//允许表格拖着
...
$("body").on("click", ".audit-operation", function() {
	...
	if (carInfoId) {
		$.ajax({
			url : basePath + "/carInfo/checkBeforeOperateCarInfo",
			asyn : false,
			type : "POST",
			data : {"carInfoId" : carInfoId,"carPart" : 1},
			dataType : "json",
 ...
 * 车辆调配
 */
$("body").on("click", ".allocate-operation", function() {
	var carInfoId = $(this).parent().attr("car-info-id");
	// 主键ID存在
	if (carInfoId) {
		$.ajax({
			url : basePath + "/carInfo/checkBeforeAllocate",
			asyn : false,
			type : "POST",
			data : {"carInfoId" : carInfoId,"carPart" : 1},
			dataType : "json",
				...
				if (dataStr) {
					if (dataStr.success) {
						...
						if (carInfoId != undefined && carInfoId != "") {
							var url = basePath+ "/carInfo/carAllocatePage";
							$("#car_allocate_info").load(url,{"carInfoId":carInfoId},function(){})
						} else {
 ...
 * 关闭车辆调配窗口
 */
$("body").on("click", ".allocate-mode-close", function() {
	$("#car_allocate_info").empty();
});
...
function checkBeforeOperate(operate) {
	...
	if (carInfoIds == undefined || carInfoIds == "") {
		...
		commonUtil.showPopup("提示", "请选择要" + operStr + "的自有车辆");
		return;
	}
	// 删除操作
	if (operate == 1) {
		...
		$.ajax({
			...
			asyn : false,
			type : "POST",
			data : {"carInfoIds" : carInfoIds,"carPart" : 1},
			dataType : "json",
			success : function(dataStr) {
				if (dataStr) {
					if (dataStr.success) {
...
function addCarInfoPage() {
	// 请求地址
	window.location.href = basePath + "/carInfo/addOrEditCarInfoPage";
}
				...
				success : function(dataStr) {
					if (dataStr) {
						if (dataStr.success) {
							if (carInfoId != undefined && carInfoId != "") {
								$("#car_info_id").val(carInfoId);
								$("#edit_form").attr("action",basePath+ "/carInfo/addOrEditCarInfoPage");
								$("#edit_form").submit();
							} else {
								commonUtil.showPopup("提示","操作的数据无效！");
							}
	...
	$
			.confirm({
				...
				content : ''
						...
						+ '<textarea type="text" placeholder="备注信息" id ="remarks" class="name form-control" required style="height:150px" />'
						+ '</div>' + '</form>',
				buttons : {
					formSubmit : {
						text : '通过',
						...
						action : function() {
							...
							if (!remarks) {
								commonUtil.showPopup("提示", "备注信息不能为空！");
								return false;
							}
							var auditInfo = {"remarks" : remarks,"carStatus" : 2,"carInfoId" : carInfoId};
							// 提交车辆审核意见信息
							$.ajax({
								url : basePath + "/carInfo/auditCarInfo",
								asyn : false,
								type : "POST",
								data : auditInfo,
								dataType : "json",
								success : function(dataStr) {
									if (dataStr) {
										...
										commonUtil.showPopup("提示","保存车辆服务异常忙，请稍后重试");
										return;
									}
								}
							});
					...
					formSubmit1 : {
						...
						action : function() {
							...
							var auditInfo = {"remarks" : remarks,"carStatus" : 3,"carInfoId" : carInfoId};
							// 提交车辆审核意见信息
							$.ajax({
								url : basePath + "/carInfo/auditCarInfo",
								asyn : false,
...
function batchAudit(carInfoIds) {
	// 标识企业内部车
	var carPart = 1;
	$.confirm({	title : '请您填写备注信息:',
				content : ''
						+ '<form action="" class="formName">'
						+ '<div class="form-group">'
						+ '<textarea type="text" placeholder="备注信息" id ="remarks" class="name form-control" required style="height:150px" />'
				...
				buttons : {
					formSubmit : {
						...
						action : function() {
							...
							var auditInfo = {"remarks" : remarks,"carStatus" : 2,"carInfoIds" : carInfoIds,"carPart" : carPart};
							// 提交车辆审核意见信息
							$.ajax({
								url : basePath + "/carInfo/batchAuditCarInfo",
								asyn : false,
								type : "POST",
								data : auditInfo,
								dataType : "json",
					...
					formSubmit1 : {
						...
						action : function() {
							...
							var auditInfo = {"remarks" : remarks,"carStatus" : 3,"carInfoIds" : carInfoIds,"carPart" : carPart};
							// 提交车辆审核意见信息
							$.ajax({
								url : basePath + "/carInfo/batchAuditCarInfo",
								asyn : false,
 ...
 * @author zhangya 2017年6月1日
 */
function saveOwnCarInfo() {
	
	var flag = xjValidate.checkForm();
	if(!flag){
		commonUtil.showPopup("提示", "请将车辆信息的必填项填写完整！");
		return;
	}
	// 1、车牌号码校验
	...
	// 43、车属单位
	var carUnit = $.trim($("#carUnit").val());
	if (carUnit == undefined || carUnit == "") {
		commonUtil.showPopup("提示", "车属单位不能为空！");
		return;
	}
	// 同意电子协议
	...
	$.ajax({
		url : basePath + "/carInfo/addOrUpdateCarInfo",
		asyn : false,
		type : "POST",
		...
		success : function(dataStr) {
			if (dataStr) {
				if (dataStr.success) {
					commonUtil.showPopup("提示", "保存成功");
					window.location.href = basePath + "/carInfo/initOwnCarInfoPage";
				} else {
					commonUtil.showPopup("提示", dataStr.msg);
					return;
				}
 ...
 * @returns
 */
function commitAllocateInfo(){
	//车辆ID校验
	var carInfoId = $.trim($("#carInfoId").val());
	if (carInfoId ==undefined || carInfoId =="") {
		commonUtil.showPopup("提示", "车辆信息无效");
		return;
	}
	//调配司机
	var currentDriver = $.trim($("#currentDriver").val());
	if (currentDriver == undefined || currentDriver == "") {
		commonUtil.showPopup("提示", "请选择要调配的司机！");
		return;
	}
	//备注校验
	var remarks = $.trim($("#remarks").val());
	...
	//原司机
	var originDriver = $.trim($("#originDriver").val());
	$.ajax({
		url : basePath + "/carInfo/carAllocate",
		asyn : false,
		type : "POST",
		data : {"carInfoId":carInfoId,"remarks":remarks,"currentDriver":currentDriver,"originDriver":originDriver},
		dataType : "json",
		success : function(dataStr) {
			if (dataStr) {
				if (dataStr.success) {
				...
				commonUtil.showPopup("提示", "车辆调配服务异常忙，请稍后重试");
				return;
			}
		}
	});
 ...
 * 显示车载影像
 */
function downloalCarImg(){
	var list = findAllCheck(".sub_check");
	if (list.length == 0) {
		commonUtil.showPopup("提示", "请选择一条数据");
		return false;
	}
	var carInfoId = list[0].id;
	
//	var path = 'group1/M00/00/3A/wKgBslnd4u6AKY5wAAEjFV5evIk897.png';
//	var filename = '新A12457';
//	$("#path").val(path);
//	$("#filename").val(filename);
//	console.log($("#downCarImg"));
//	var url = basePath + "/carInfo/downCarImg";
//    $('#downCarImg').attr('action', url);
//    $('#downCarImg').submit();
//    return false;
//    
//	$.ajax({
//		url:basePath + "/carInfo/downCarImg",
//		async: false,
//		type:'POST',
...
//	return false;
	...
	// 查找车辆信息
	$.ajax({
		url : basePath + "/carInfo/getCarInfoById",
		async : false,
		type : "POST",
		data : {"carInfoId":carInfoId},
		dataType : "json",
	...
	var operationCertImg = carInfo.operationCertImg;
	var driverLicenseImg = carInfo.driverLicenseImg;
	var driverLicenseImgCopy = carInfo.driverLicenseImgCopy;
	var carCode = carInfo.carCode;
	
	console.log(operationCertImg);
	console.log(driverLicenseImg);
	console.log(driverLicenseImgCopy);
	console.log(carCode);
	$("#path").val(operationCertImg);
	$("#filename").val(carCode);
	var url = basePath + "/carInfo/downCarImg";
    $('#downCarImg').attr('action', url);
    $('#downCarImg').submit();
    
//    $("#path").val(driverLicenseImg);
//	$("#filename").val(carCode);
//	var url = basePath + "/carInfo/downCarImg";
//    $('#downCarImg').attr('action', url);
//    $('#downCarImg').submit();
//    
//    $("#path").val(driverLicenseImgCopy);
//	$("#filename").val(carCode);
//	var url = basePath + "/carInfo/downCarImg";
//    $('#downCarImg').attr('action', url);
//    $('#downCarImg').submit();
