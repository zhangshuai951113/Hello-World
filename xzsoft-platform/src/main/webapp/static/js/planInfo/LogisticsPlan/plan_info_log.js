//日志操作查看
function findAllLogMation(id){
	$("#planInfoLogModel").modal("show");
	planInfoLogList(1,id);
}

//查询计划日志信息
function planInfoLogList(number,id){
	
	//禁止多次追加
	$("#planInfologTBody").html("");
	
	$.ajax({
		url:basePath+"/planInfo/findPlanInfoLogAllMation",
		...
		success:function(resp){
			...
			$.each(objs, function(index, ele) {
				...
				// 将tr追加
				$("#planInfologTBody").append(tr);
			});
		}
	});
	// 获取最大数据记录数
	$.ajax({
		url : basePath+"/planInfo/getPlanInfoLogAllMationCount",
		type : "post",
		data : {"id":id},
		dataType : "json",
		async : false,
		success : function(resp) {
			...
			planInfoLogPagination.setTotalItems(resp);
			$("#planInfologNum").text(resp);
		}
	});
}
//分页
var planInfoLogPagination = $(".plan-info-log").operationList({
	"current" : 1, // 当前目标
	"maxSize" : 4, // 前后最大列表
	"itemPage" : 10, // 每页显示多少条
	"totalItems" : 0, // 总条数
	"chagePage" : function(current) {
		// 调用ajax请求拿最新数据
		planInfoLogList(current);
	}
});

function jumpPageNum(e) {
