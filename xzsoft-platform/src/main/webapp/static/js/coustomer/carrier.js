...
//添加模态框
function addCurrierMation(){
	var url=basePath+"/customer/addCurrierMation";
...
function list(number){
	...
	data+="&page="+(number-1)+"&rows="+10+"&sign="+2+"";
	//查询组织机构明细
	$.ajax({
...
function updateCarrierMation(){
	...
	var userIdsLength=userIds.length;
	updateCurrierMation(userIds.join(","),userIdsLength);
}

//修改客户数据
function updateCurrierMation(userIds,userIdsLength){
			...
			success:function(resp){
				if(resp){
					if(resp.success){
						...
						//弹出修改模态框
						var url=basePath+"/customer/updateCarrierPage";
						$("#show_coustomer_info").load(url,function(){
							//上传图片初始化
							$('.upload_img').each(function(){
								uploadLoadFile($(this));
