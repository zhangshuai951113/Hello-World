...
$("body").on("click",".detail-edit",function() {
	...
	if (contractInfoId) {
		...
		$("#contract_detail_form").attr("action",basePath + "/contractDetailInfo/showContractDetailInfoPage");
...
/**
 * 管理合同操作
 * @param contractInfoIds 合同ID
 * @param operateType (操作类型 2:提交审核 5:发送确认 6:撤回)
 * @returns
 */
...
/**
 * 操作选中的合同
 * @param operateType(操作合同信息 2 提交审核 5 发送确认 6 撤回)
 * @returns
 */
 ...
 * 终止合同
 */
function terminationContract(){
	//TODO
	var conIds=findAllCheckedContractInfoIds();
	$.confirm({
		title : "提示",
		content : "是否确认终止所选合同信息!",
		buttons : {
			'确认' : function() {
				$.ajax({
					url : basePath + "/contractInfo/terminationContract",
					asyn : false,
					type : "POST",
					data : {
						"contractInfoIds" : conIds
					},
					dataType : "json",
					success : function(dataStr) {
						if (dataStr) {
							...
							commonUtil.showPopup("提示", "服务异常，请稍后重试");
							return;
						}
					}
				});
...
function saveContractInfo() {
	...
	$.ajax({
		url : basePath + "/contractInfo/addOrUpdateContractInfo",
		asyn : false,
		type : "POST",
...
function cooperateStatusChange(){
	$("#entrust").val(null);
	$("#entrust_name").val(null);
	$("#shipper").val(null);
	$("#shipper_name").val(null);
	$("#entrust_org_root").val(null);
	$("#entrust_org_root_name").val(null);
