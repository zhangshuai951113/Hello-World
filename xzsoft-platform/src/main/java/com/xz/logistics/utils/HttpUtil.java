...
public class HttpUtil {
	 ...
	 *             发送异常
	 */
	public static USendResponse send(TsContext context, String reqXml)
			throws Exception {
		...
		sendReq.setToaddress(context.getProperty("toAddress").split(","));
		sendReq.setSendRequestTypeXML("LOGINK_CN_FREIGHTBROKER_WAYBILL", reqXml);
		transporter = ITransportClientFactory.createMessageTransporter(
				context.getUerToken(), context.getProperty("targetURL"));
		long start = System.currentTimeMillis();
		try {
			response = transporter.send(sendReq);
			long end = System.currentTimeMillis();
			System.out.println("开始时间" + (end - start) + "结束时间"
					+ reqXml.getBytes().length);
		} catch (Exception e) {
			throw new Exception("连接无车承运人监测服务器失败！！");
		}
		return response;
