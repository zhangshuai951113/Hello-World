...
var proxyPage = null;
var fromDriver = "distributeDriver"; //判断选择是哪种派发，司机派发直接选择，代理派发需要代理司机选择
...
function proxyDistributeModalSave(){
	...
	// 运价表id
	var transportPriceId = $.trim($("#proxyDistributeModal_transportPriceId").val());
	// 司机id
	var driverInfoId = $.trim($("#proxyDistributeModal_driverInfoId").val());
	// 运价
	//var transportPrice = $.trim($("#proxyDistributeModal_transportPrice").val());
	// 司机角色 
	var driverUserRole = tempDriverUserRole;
	...
	/*
	if (driverUserRole == 3) {
		if (commonIsEmpty(transportPrice)) {
			xjValidate.showTip("请输入运价");
			return;
		} else {
			if (!checkFffds(transportPrice)){
				xjValidate.showTip("请输入非负浮点数");
				return ;
			}
		}
	}
...
function getWaybillList(){
	...
	$.ajax({
		url : basePath + "/waybillInfo/getLogisticsReceiveWaybillInfoList",
		asyn : false,
		type : "POST",
		data : params,
		dataType : "json",
			...
			if (data) {
				if (data.success) {
					...
					for(var i = 0; i < rows.length; i++){
						...
						html += '<td>';
						html += '<label class="i-checks">';
						html += '<input data-id="'+info.id+'" data-name="委托方" class="proxy-check" type="checkbox">';
						html += '</label>';
						html += '</td>';
						html += '<td>'+info.entrustName+'</td>';
						html += '<td>'+info.goodsName+'</td>';
						html += '<td>'+info.lineName+'</td>';
						html += '<td>'+info.forwardingUnit+'</td>';
						html += '<td>'+info.consignee+'</td>';
						html += '<td>'+new Date(info.startDate).Format("yyyy-MM-dd")+'</td>';
						html += '<td>'+info.unitPrice+'</td>';
						html += '</tr>';
					}
