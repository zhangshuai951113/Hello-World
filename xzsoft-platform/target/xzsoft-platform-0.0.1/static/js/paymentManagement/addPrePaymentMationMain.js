...
//取消
function closeAddPrePaymentInfo(){
	window.location.href=basePath+"/paymentInfo/goPrePaymentInfoMationPage";
}
...
//点击保存按钮
function addPrePaymentInfoMation(){
	sign=1;
	addOrUpdatePrePaymentInfoMation(sign);
}


function addOrUpdatePrePaymentInfoMation(sign){
	
	//验证
	//1、预付金额
	var prePaymentPrice=$.trim($("#prePaymentPrice").val());
	if(prePaymentPrice==undefined || prePaymentPrice==''){
		 xjValidate.showPopup("请输入预付金额!","提示",true);
		 return false;
	}
	...
	if(!regax.test(prePaymentPrice)){
		 xjValidate.showPopup("请输入正确的预付金额!","提示",true);
		return false;
	}
	...
	if(waybillInfoName==undefined || waybillInfoName==''){
		 xjValidate.showPopup("请选择运单信息!","提示",true);
		 return false;
	}
	...
	if(paymentType==undefined || paymentType==''){
		 xjValidate.showPopup("请选择支付类型!","提示",true);
		 return false;
	}
	...
	$.ajax({
		...
		data:$("#add_pre_payment_info_form").serialize()+"&sign="+sign+"",
		dataType:"json",
		type:"post",
		async:false,
