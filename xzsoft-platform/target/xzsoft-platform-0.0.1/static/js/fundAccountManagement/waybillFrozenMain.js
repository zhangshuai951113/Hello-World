$(function(){
    ...
    wayFrozenList(1);
  //全选/全不选
	$("body").on("click",".all_way_frozen",function(){
		if($(".all_way_frozen").is(":checked")){
			//全选时
			$(".sub_way_frozen").each(function(){
...
function wayFrozenList(number){
	...
	$.ajax({
		...
		success:function(resp){
			...
			$.each(obj,function(index,ele){
				...
				tr+="<td><label class='i-checks'><input name='cId' id='id' type='checkbox' class='sub_way_frozen' data-id=\""+ele.id+"\" ></label></td>";
				tr+="<td>"+(index+1)+"</td>";
	...
	$.ajax({
		...
		success:function(resp){
			...
			$("#way_frozen_num").text("搜索结果共"+resp+"条");
		}
	});
}
...
//运单被冻结信息导出
function wayFreezeInformationExport(){
	 ...
	 var ids=getCapIds();
		...
		var url = basePath+ "/fundAccountManagement/wayFreezeInformationExport?ids="+ids+"&sign="+1;
