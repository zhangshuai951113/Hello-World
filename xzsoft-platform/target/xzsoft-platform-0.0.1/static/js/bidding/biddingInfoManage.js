$(function() {
	...
	// 明细管理页
	$("body").on("click", ".detail-operation", function() {
		// 招标信息ID
		var biddingInfoId = $(this).parent().attr("bidding-info-id");
		if (biddingInfoId && biddingInfoId != "") {
			$("#bidding_info_id").val(biddingInfoId);
			$("#bidding_detail_form").attr("action",basePath + "/biddingDetailInfo/showBiddingDetailInfoPage");
			$("#bidding_detail_form").submit();
		}else{
	...
	$("body").on("click", ".modify-operation", function() {
		...
		var biddingInfoId = $(this).parent().attr("bidding-info-id");
		if (biddingInfoId && biddingInfoId != "") {
			addOrEditBiddingInfoPage(biddingInfoId);
		}else{
			commonUtil.showPopup("提示", "非法操作！");
			return;
		}
	...
	$("body").on("click", ".delete-operation", function() {
		...
		if (biddingInfoId && biddingInfoId != "") {
			$.confirm({
				title : "提示",
				content : "是否确认删除将所选招标信息?",
				buttons : {
					'确认' : function() {
						operateBiddingInfo(biddingInfoId,3);
					},
					'取消' : function() {
	...
	// 招标信息二次挂网
	$("body").on("click", ".on-line-operation", function() {
		// 招标信息ID
		var biddingInfoId = $(this).parent().attr("bidding-info-id");
		if (biddingInfoId && biddingInfoId != "") {
			biddingInfoReleasePage(biddingInfoId);
		}else{
			commonUtil.showPopup("提示", "非法操作！");
			return;
		}
 ...
 * @param number 页数
 */
function searchBiddingInfo(number) {
	// 招标信息编号
	var biddingCode = $.trim($("#biddingId").val());
	// 招标信息名称
	var biddingName = $.trim($("#biddingName").val());
	var orgInfoId = $.trim($("#orgInfoId").val());
	// 请求地址
	var url = basePath + "/biddingInfo/listBiddingInfoPage #bidding-info-data";
 ...
 * @returns 用户ID，逗号分隔
 */
function findAllCheckBiddingInfoIds() {
	// 所有选中招标信息ID
 ...
 * @author zhangya 2017年6月12日
 */
function addOrEditBiddingInfoPage(biddingInfoId) {
	$("#biddingModal").modal("show");
	// 请求地址
	var url = basePath + "/biddingInfo/editBiddingInfoPage #bidding-content";
...
function saveBiddingInfo() {
	...
	// 招标信息编号
	var biddingCodeCopy = $("#bidding_code_copy").val();
	if (biddingCodeCopy == undefined || biddingCodeCopy == "") {
		commonUtil.showPopup("提示", "招标信息编号不能为空");
		return;
	}
...
//	}
	...
	// 保证金金额
	var cautionMoney = $("#cautionMoney").val();
	if (cautionMoney == undefined || cautionMoney == "") {
		commonUtil.showPopup("提示", "请输入保证金金额!");
		return;
	}
	
	$.ajax({
		url : basePath + "/biddingInfo/saveBiddingInfo",
		asyn : false,
		type : "POST",
 ...
 * @returns
 */
function operateCheckedBiddingInfo(operateType) {
	// 校验招标信息ID
	...
	//获取选中的ID
	var biddingInfoIds = findAllCheckBiddingInfoIds();
	...
	if(operateType !=2 ){
		$.confirm({
			...
			buttons : {
				'确认' : function() {
					operateBiddingInfo(biddingInfoIds,operateType);
				},
				'取消' : function() {
 ...
 * @returns
 */
function operateBiddingInfo(biddingInfoIds,operateType){
	
	$.ajax({
		url : basePath + "/biddingInfo/operateBiddingInfo",
		asyn : false,
		type : "POST",
		data : {
				...
				buttons : {
					...
					formSubmit1 : {
						...
						action : function() {
							...
							// 提交审核信息
							biddingInfoAudit(biddingInfoIds, opinion, 2,operateType)
						}
					},
					'取消' : function() {
						// close
...
function biddingInfoAudit(biddingInfoIds, opinion, result,operateType) {
	...
	// 提交主机构审核意见信息
	$.ajax({
		url : basePath + "/biddingInfo/auditBiddingInfo",
		asyn : false,
		type : "POST",
		data : auditInfo,
		dataType : "json",
 ...
 * 保存招标信息二次挂网
 */
function saveBiddingInfoForRelease() {
...
//	}
	...
	$.ajax({
		url : basePath + "/biddingInfo/saveBiddingInfoForRelease",
		asyn : false,
		type : "POST",
		data : $('#bidding_info_form').serialize(),
		dataType : "json",
...
function biddingInfoReleasePage(biddingInfoId) {
	$("#biddingModal").modal("show");
	// 请求地址
	var url = basePath + "/biddingInfo/editBiddingInfoPageForRelease #bidding-content";
	$("#show_bidding_info").load(url, {
		"biddingInfoId" : biddingInfoId
	}, function() {
		// 上传图片初始化
